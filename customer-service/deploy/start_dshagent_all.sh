...
#   游客端（对话）: http://127.0.0.1:${GUEST_PORT:-10800}/
#   Admin 管理端   : http://127.0.0.1:${ADMIN_PORT:-10801}/
...
for i in $(seq 1 40); do
  if curl -s -o /dev/null "http://127.0.0.1:${GUEST_PORT}/api/guest/health" 2>/dev/null; then
    READY=1; break
  fi
  sleep 3
done
...
if [ "$READY" = "1" ]; then
  ...
  echo ""
  echo "  🌐 游客端（智能客服对话）: http://127.0.0.1:${GUEST_PORT}/"
  echo "  🔐 Admin 管理端          : http://127.0.0.1:${ADMIN_PORT}/"
  echo ""
