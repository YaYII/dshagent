...
`/api/contract/all/` 的名字有误导性。它不是「给客服列出该客户的合约」，
而是**「核对客人报出的合约号」**：

- 客人知道自己的合约号（在账单/卡片上），客服的角色是**确认**，不是**告知**。
- 类比 10086：热线不会把你的电话号码念给你听，而是让你报号、它来核对。
- 该接口一次返回账户下**全部** 151 笔合约（含姓名、地址、缴费状态）。若让模型直接
  读它，等于给访客一个「一次调用即可 dump 整份客户清单」的枚举能力。

因此系统在结构上而不是提示词上做了限制（详见第 8 节）：

| 通道 | 能否触达 `/api/contract/all/` | 模型能看到什么 |
|---|---|---|
| `api_get`（模型自拼 URL） | ❌ 路径白名单已排除该前缀 | 什么也拿不到 |
| `lookup_contract`（插件按模板拼 URL） | ✅ 唯一通道 | 只有 `found` + **该一个合约号自己的记录** |

模型既拿不到其它合约，也拿不到 `no_ca` 总数，**所以在设计上不可能报出「您名下共有
N 张合约」**。演示环境下这个边界同样生效，无需额外配置。
