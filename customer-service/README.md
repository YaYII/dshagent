...
- **缴费付款码**：账单查询用 `level=5` 取回付款码（接口字段名 `barcode`，30 位数字）。模型把它放进
  ` ```barcode ` 围栏，前端渲染成**一张二维码**，并附一键复制。
  - 只出二维码、不出条码，正文里那串数字也隐去（二维码已含同一串数字，重摆一遍会让访客以为要
    手动输）。数字仍在一键复制里，扫不出来时有退路。
  - 标签指向**「澳電 CEM」App**（本澳缴费入口是澳电自家 App），不指向微信／支付宝——这是业务口径，
    提示词里也写死了，避免模型自行发挥。
  - 二维码用随页面部署的 `qrcode-generator`（MIT，与 mermaid 同一套懒加载形态）；库不可用时只少
    一个码图，卡片与复制按钮照常——库缺失不该让访客拿不到付款码。
  - 识别不问模型：` ```barcode ` 围栏、行内代码、句子里直接写数字三种形态都会出二维码；句子里那种
    只隐掉数字本身，同句的金额与提示照常可见。
  - 判据是**真的解一次**而不是「图出来了」：把渲染出的 SVG 读回模块矩阵、合成像素，交给独立解码器
    jsQR 解，必须解出**业务系统接口当前返回的**付款码（期望值现取，不写死）——编码错了不会报错，
    只会让访客缴错费（`web/guest/tests/paycode-check.mjs`）。

  ![缴费付款码二维码](docs/paycode-card.png)
